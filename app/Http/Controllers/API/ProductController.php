<?php

namespace App\Http\Controllers\API;

use App\Helpers\ResponseFormatter;
use App\Models\Product;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;

class ProductController extends Controller
{
    public function all(Request $request)
    {
        $id = $request->input('id');
        $limit = $request->input('limit');
        $name = $request->input('name');
        $description = $request->input('description');
        $tags = $request->input('tags');
        $categories = $request->input('categories');
        $price_from = $request->input('price_from');
        $price_to = $request->input('price_to');

        if($id){
            $product = Product::with(['category', 'galleries'])->find($id);
            
            if($product){
                return ResponseFormatter::success($product, 'Data berhasil diambil');
            }else{
                return ResponseFormatter::error(null, 'Data produk tidak ada', 404);
            }
        }

        $product = Product::with(['category', 'galleries']);

        // Kondisi jika mencari berdasar nama
        if($name){
            $product->where('name', 'like', '%'.$name.'%');
        }
        // Kondisi jika mencari berdasar description
        if($description){
            $product->where('description', 'like', '%'.$description.'%');
        }
        // Kondisi jika mencari berdasar tags
        if($tags){
            $product->where('tags', 'like', '%'.$tags.'%');
        }
        // Kondisi jika mencari berdasar price_from
        if($price_from){
            $product->where('price_from', '>=', $price_from);
        }
        // Kondisi jika mencari berdasar price_from
        if($price_to){
            $product->where('price_to', '<=', $price_to);
        }
        // Kondisi jika mencari berdasar categories
        if($price_to){
            $product->where('category', $categories);
        }

        return ResponseFormatter::success($product->paginate($limit), 'Data produk berhasil diambil');
    }
}
